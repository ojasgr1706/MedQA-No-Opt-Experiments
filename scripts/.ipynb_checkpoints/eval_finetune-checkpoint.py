...
def main():
    ...
    if load_checkpoint:
        ...
        if args.load_checkpoint_epoch:
            Path = f'{args.output_dir}{model_name}_finetuned_resp_epoch{load_checkpoint}.pth'
    ...
    log_file = "eval_finetune_log.txt"
    
    bertscore_metric = load('bertscore',experiment_id='bs.'+log_file.replace("/", ""))
...
#         out = model.generate(batch['text_ids'], max_new_tokens = args.max_new_tokens)
#         out = out[:,len(batch['text_ids']):]
...
#         orig = tokenizer.batch_decode(out, skip_special_tokens = True)
#         out = out[:,len(batch['text_ids'][0]):]
...
#         results_bert.append(results['f1'])
#         print(results_bert[-1])
#         progress_bar.update(1)

        # elem = torch.cat((batch['text_ids'], batch['label_ids']),dim=1)
    
        # loss = model(input_ids = batch['text_ids'], labels = batch['label_ids']).loss
