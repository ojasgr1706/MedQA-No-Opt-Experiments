...
def evaluation(args, dataset, mixed_precision="fp16"):
    ...
    resp_dataset = dataset['resp']
    endo_dataset = dataset['endo']
    resp_dataloader = DataLoader(resp_dataset['test'], shuffle=True, batch_size=test_batch_size)
    endo_dataloader = DataLoader(endo_dataset['test'], shuffle=True, batch_size=test_batch_size)
    model, resp_dataloader, endo_dataloader = accelerator.prepare(model, resp_dataloader, endo_dataloader)
    ...
    progress_bar = tqdm(range(args.n_samples))
    epoch_losses = []
    ...
    # df_outputs = pd.DataFrame(columns = ['resp_gold','resp_predicted','endo_gold','endo_predicted'])
    df_outputs = pd.DataFrame(columns = ['resp_gold','resp_predicted'])
    resp_it = iter(resp_dataloader)
    # endo_it = iter(endo_dataloader)
    ...
    for i in range(args.n_samples):
        ...
        preds_resp = tokenizer.batch_decode(gen_out, skip_special_tokens = True)
        gold_resp = tokenizer.batch_decode(targets, skip_special_tokens = True)
...
#         preds_endo = tokenizer.batch_decode(gen_out, skip_special_tokens = True)
#         gold_endo = tokenizer.batch_decode(targets, skip_special_tokens = True)
            
        # df_outputs.loc[len(df_outputs)] = [gold_resp[0],preds_resp[0],gold_endo[0],preds_endo[0]]
        df_outputs.loc[len(df_outputs)] = [gold_resp[0],preds_resp[0]]
...
def main():
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument("--train_data_resp", help="Add respiratory system data files (single file name or list fo files in the format : [a,b,c,...]. The files in the list will be concatenated before being used as training data)", required=True, type=str_or_list)
    parser.add_argument("--train_data_endo", help="Add endocrine system data files (single file name or list fo files in the format : [a,b,c,...]. The files in the list will be concatenated before being used as training data)", required=True, type=str_or_list)
    ...
    parser.add_argument("--n_samples", help="samples from each domain", required=False, type=int, default=10)

    args = parser.parse_args()

    resp_dataset = load_dataset('text',data_files={'test': args.train_data_resp})
    endo_dataset = load_dataset('text',data_files={'test': args.train_data_endo})
    ...
    tokenized_dataset_resp = resp_dataset.map(
        tokenize, batched=True, remove_columns=resp_dataset["test"].column_names
    )
    
    tokenized_dataset_endo = endo_dataset.map(
        tokenize, batched=True, remove_columns=endo_dataset["test"].column_names
    )
    
    # pdb.set_trace()

    tokenized_dataset_resp.set_format("torch")
    tokenized_dataset_endo.set_format("torch")
    
    print(tokenized_dataset_resp)
    print(tokenized_dataset_endo)

    os.environ["TOKENIZERS_PARALLELISM"] = 'false'
    
    evaluation(args, {"resp":tokenized_dataset_resp,"endo":tokenized_dataset_endo})
