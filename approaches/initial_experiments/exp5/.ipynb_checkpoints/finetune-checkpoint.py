...
def training_loop(args, dataset, mixed_precision="fp16"):
    ...
    if load_checkpoint:
        Path = f'{args.output_dir}{model_name}_finetuned_exp5_epoch{load_checkpoint}.pth'
        ckpt_dict = torch.load(Path)
        model.load_state_dict(ckpt_dict['model_state_dict'])
    ...
    for epoch in range(args.load_checkpoint_epoch,args.load_checkpoint_epoch + num_epochs):
        ...
        with open(f"{args.output_dir}exp5_finetune_logs.txt","a") as f:
        ...
        if(checkpoint and (epoch+1)%10 == 0):
            ...
            for step,test_batch in enumerate(test_dataloader, start = 1):
                ...
                with torch.no_grad():
                    test_loss = model(inputs, labels = labels, attention_mask = attention_mask).loss
            ...
            Path = f'{args.output_dir}/{model_name}_finetuned_exp5_epoch{epoch+1}.pth'
