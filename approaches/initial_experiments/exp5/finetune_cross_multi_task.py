...
def training_loop(args, tok_len, dataset, seed, mixed_precision="fp16"):
    ...
    for epoch in range(args.load_checkpoint_epoch,args.load_checkpoint_epoch + num_epochs):
        ...
        if(checkpoint and (epoch+1)%10 == 0):
            ...
            if((epoch+1)%40 == 0 or (epoch+1)%50 == 0):
                ...
                Path = f'{args.output_dir}/{model_name}_multi_task_finetuned_exp5_epoch{epoch+1}_seed{seed}.pth'
...
def main():
    ...
    for seed in seeds:
        ...
        qa_train, qa_test = train_test_split(data, test_size=0.15, random_state=seed)
        ...
        training_loop(args, len(tokenizer), {"qa":qa_tokenized_datasets, "text":text_tokenized_datasets}, seed)
