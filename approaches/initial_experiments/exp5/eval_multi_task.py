...
def main():
    ...
    for checkpoint in range(10,81,10):
        Path = f'{args.output_dir}{model_name}_multi_task_finetuned_exp5_epoch{checkpoint}.pth'
...
#             ppl = np.exp(loss)
        ...
        print(f"mean_rougeL : {mean_rouge} , mean_bertscore : {mean_bert} , checkpoint : {checkpoint}")
            
        iodf[f'checkpoint_{checkpoint}'] = predictions
        results_df[f'checkpoint_{checkpoint}'] = [mean_rouge,mean_bert]
        
    iodf.to_csv(f"{args.output_dir}exp5_multi_task_finetuning_io.csv")
    results_df.to_csv(f"{args.output_dir}exp5_multi_task_finetuning_scores.csv")
