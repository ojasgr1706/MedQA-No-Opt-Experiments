...
def main():
    ...
    parser.add_argument("--qa", help="Perform evaluation on qa", required=False, action="store_false")
    parser.add_argument("--doc_com", help="Perform evaluation on document completion", required=False, action="store_true")
    ...
    for seed in seeds:
        ...
        _, data = train_test_split(full_data, test_size=0.15, random_state=seed)
        ...
        for checkpoint in checkpoints:
            Path = f'{args.output_dir}{model_name}_multi_task_finetuned_exp5_epoch{checkpoint}_seed{seed}.pth'
    ...
    #             ppl = np.exp(loss)
            ...
            print(f"\nModel seed : {seed}, mean_rougeL : {mean_rouge} , mean_bertscore : {mean_bert} , checkpoint : {checkpoint}")

            iodf[f'checkpoint_{checkpoint}'] = predictions
            results_df[f'checkpoint_{checkpoint}'] = [mean_rouge,mean_bert]

        iodf.to_csv(f"{args.output_dir}exp5_multi_task_finetuning_seed{seed}_io.csv")
        results_df.to_csv(f"{args.output_dir}exp5_multi_task_finetuning_seed{seed}_scores.csv")
