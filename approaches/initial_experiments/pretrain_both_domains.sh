...
mkdir -p "${output_dir}"
cmd="accelerate launch --num_processes=${num_gpus} pretrain_both_domains.py --model_name ${model_name} --tk_name ${tokenizer} --train_data ${train_data} --output_dir ${output_dir} --load_checkpoint_epoch ${checkpoint}"
