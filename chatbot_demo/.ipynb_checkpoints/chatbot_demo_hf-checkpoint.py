...
#             op_text, op_reasoning = text_output.split(reasoning_delimiter)
#             op_text, op_reasoning = op_text.strip(), op_reasoning.strip()
...
# if option == "MedCodex + Codex (F+B)" or option == "MedCodex + Verifier (F + RM)":
...
#     uniq_options, op_to_reason = create_options(f_instringinstring)
#     if (len(uniq_options) < 4):
#         # output = "Not enough options generated!"
#         st.write("Not enough Options")

#     else:
#         if option == "MedCodex + Codex (F+B)":
...
#             b_answer_idx = ord(b_answer) - ord('A')
#             output = b_answer + op_to_reason[uniq_options[b_answer_idx]] + " " + uniq_options[b_answer_idx]

#         elif option == "MedCodex + Verifier (F + RM)":
...
#             for k in range(4):
#                 instring = f"Question : {user_input}\nReasoning : Let's think step by step. {op_to_reason[uniq_options[i]]}\nAnswer : {uniq_options[i]}"
...
#             answer = chr(answer_idx + ord('A'))

#             output = answer + " " + op_to_reason[uniq_options[answer_idx]] + " " + uniq_options[answer_idx]
